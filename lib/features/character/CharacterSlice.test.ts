...
describe("CharacterSlice — applyQuestEffect", () => {
  ...
  it("applies coin reward", () => {
    const store = storeWithCharacter();
    store.dispatch(
      applyQuestEffect({ coins: { gold: 5, silver: 0, copper: 0 } }),
    );
    expect(
      store.getState().character.ActiveCharacter?.inventory.coins.gold,
    ).toBe(15);
  });
  ...
  it("coin values cannot go below 0", () => {
    const store = storeWithCharacter();
    store.dispatch(
      applyQuestEffect({ coins: { gold: -100, silver: -100, copper: -100 } }),
    );
  ...
  it("adds quest item to questItems inventory", () => {
    ...
    expect(
      store.getState().character.ActiveCharacter?.inventory.questItems,
    ).toHaveLength(1);
    expect(
      store.getState().character.ActiveCharacter?.inventory.questItems[0].id,
    ).toBe("relic-1");
  });
  ...
  it("adds item to correct inventory category", () => {
    ...
    store.dispatch(applyQuestEffect(effect));
    expect(
      store.getState().character.ActiveCharacter?.inventory.potions,
    ).toHaveLength(1);
  });
