...
type InventoryItemModalProps = {
  ...
  handleInventorySelect: (
    inventoryOption: BattleOption | InventoryItemBase,
  ) => void;
};
...
export const InventoryItemModal = ({
  ...
  if (
    ("battleOptionType" in inventoryItem &&
      inventoryItem.battleOptionType === "attack") ||
    ("battleOptionType" in inventoryItem &&
      inventoryItem?.battleOptionType === "skill")
  ) {
    modalType = "read-only";
  } else if (
    inventoryItem?.type === "potion" ||
    inventoryItem?.type === "ration"
  ) {
    actionButtonLabel = "Use";
    modalType = "action";
  } else if (
    inventoryItem?.type === "weapon" ||
    inventoryItem?.type === "equipment"
  ) {
