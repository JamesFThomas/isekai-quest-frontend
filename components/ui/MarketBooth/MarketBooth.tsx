...
export const MarketBooth = () => {
  ...
  const [selectedBoothItem, setSelectedBoothItem] =
    useState<InventoryItemBase | null>(null);
  ...
  const handleBoothItemClick = (item: InventoryItemBase) => {
    const selectedItem = boothItems.find(
      (boothItem) => boothItem.id === item.id,
    );
  ...
  const handlePurchaseClick = (item: InventoryItemBase) => {
    ...
    // dispatch item purchased toast
    dispatch(
      addToast([
        {
          id: `item_purchased_${item.id}`,
          ...
          >
            {boothItems.map((item) => (
              <button
                ...
                disabled={
                  !item.price ||
                  !canAffordItem(activeCharacter?.inventory?.coins, item.price)
                } // disable if no price or cannot afford
              ...
              >
                ...
                <span className="mt-2 text-sm text-white font-bold text-center">
                  {item.price ? (
                    <div>{formatPriceDisplay(item.price).label}</div>
                  ) : null}
                </span>
              </button>
            ))}
